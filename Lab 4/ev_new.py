...
Outputs per country:
 - combined plot (S-curve cumulative % + bell curve adoption rate %)
 - residuals plot
...
# -------------------------
def fit_country(df_country, country_name, out_dir="fits_combined", plot=True):
    ...
    if plot:
        ...
        # --- Combined plot: cumulative % (S-curve) + adoption rate % (bell curve) ---
        plt.figure(figsize=(7,4.5))

        years_dense = np.linspace(min(years), max(years) + 10, 300)
        t_dense = years_dense - 2010

        # cumulative (S-curve), normalized
        N_dense = bass_cumulative(t_dense, p_fit, q_fit, C_fit)
        frac_dense = N_dense / C_fit * 100

        # adoption rate (bell curve), normalized
        x_dense = N_dense / C_fit
        r_dense = (p_fit + q_fit * x_dense) * (1 - x_dense) * C_fit
        r_frac_dense = r_dense / C_fit * 100  # % of market potential per year

        plt.plot(years_dense, frac_dense, label="Cumulative adoption (S-curve)", color="C0", lw=2)
        plt.plot(years_dense, r_frac_dense, label="Adoption rate (Bell curve)", color="C1", lw=2)

        # actual data (normalized)
        plt.scatter(years, Ndata / C_fit * 100, color="C0", alpha=0.7, label="Data (cumulative %)")

        # annotate peak year if available
        if not np.isnan(peak_year):
            frac_peak = bass_cumulative(t_star_rel, p_fit, q_fit, C_fit) / C_fit * 100
            plt.axvline(peak_year, color="red", linestyle="--", alpha=0.7)
            plt.text(peak_year, 90, f"Peak ~{peak_year:.1f}", color="red")

        plt.title(f"{country_name}: Bass fit (normalized)")
        plt.xlabel("Year")
        plt.ylabel("% of market potential")
        plt.ylim(0, 105)
        plt.legend()
        plt.grid(alpha=0.3)
        fname = os.path.join(out_dir, f"{country_name}_curves.png")
...
def main():
    ...
    if results:
        ...
        df_res.to_csv("bass_fits_summary_new.csv", index=False)
        print("\nSaved summary to bass_fits_summary_new.csv")
    else:
